...
| 명령어 | 설명 |
|--------|------|
| `/coupon <코드> [코드 ...]` | 등록된 **모든 ID**에 쿠폰을 적용. 처리 끝나면 성공/실패 정리한 결과를 한 번에 회신 |
| `/add <ID> [ID ...]` | ID 추가 (공백/콤마/줄바꿈 다 구분자로 가능) |
| `/del <ID> [ID ...]` | ID 삭제 |
| `/list` | 등록된 ID 목록 |
| `/server <KR/JP/GLB>` | 기본 서버 변경 (현재 값 조회는 인자 없이) |
| `/help` | 도움말 |

**여러 개 한 번에:** `/add user1 user2,user3` 또는 줄바꿈으로 보내도 됨. `/coupon`, `/del` 도 동일.

**엑셀 일괄 등록:** 봇에게 `.xlsx` 파일을 첨부해서 보내면 A열 2행부터 ID로 자동 추가됩니다 (1행은 헤더, 기존 GUI 버전 포맷 그대로).
...
- exe를 실행하면 **콘솔 창이 뜨지 않고** 시스템 트레이(우측 하단 알림 영역)에 파란 아이콘이 생깁니다.
- 트레이 아이콘 **우클릭**:
  - `로그 열기` — `bot.log` 를 메모장으로 열어 실시간 로그 확인
  - `종료` — 봇 종료
- 시작 실패(.env 누락 등) 시에는 Windows 메시지 박스로 알림.
